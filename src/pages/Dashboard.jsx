      ...
      <Grid container spacing={4}>
          ...
          <StatCard elevation={darkMode ? 0 : 2}>
            <CardContent sx={{ p: 3 }}>
              <Box display="flex" alignItems="center" justifyContent="space-between" mb={3}>
                <Typography variant="h6" fontWeight={600}>شیفت‌های این ماه</Typography>
                <IconWrapper>
                  <AccessTimeIcon />
                </IconWrapper>
              </Box>
              <Typography variant="h3" fontWeight={800}>{stats?.monthlyShifts || 0}</Typography>
            </CardContent>
          </StatCard>
        </Grid>
        ...
        <Grid item xs={12} md={3}>
          <StatCard elevation={darkMode ? 0 : 2}>
            <CardContent sx={{ p: 3 }}>
              <Box display="flex" alignItems="center" justifyContent="space-between" mb={3}>
                <Typography variant="h6" fontWeight={600}>ساعات کاری</Typography>
                <IconWrapper>
                  <TrendingUpIcon />
                </IconWrapper>
              </Box>
              <Typography variant="h3" fontWeight={800}>{stats?.totalHours || 0}</Typography>
            </CardContent>
          </StatCard>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <StyledPaper elevation={darkMode ? 0 : 1}>
            <Box display="flex" alignItems="center" mb={3}>
              <CalendarIcon sx={{ ml: 2, color: 'primary.main' }} />
              <Typography variant="h6" fontWeight={600}>شیفت‌های آینده</Typography>
            </Box>
            {stats?.upcomingShifts?.map((shift, index) => (
              <NotificationBox key={index}>
                <Typography sx={{ fontSize: '1.1rem', fontWeight: 500 }}>
                  {moment(shift.date).format('jYYYY/jMM/jDD')} - {shift.type}
                </Typography>
              </NotificationBox>
            ))}
          </StyledPaper>
